...
gdata <- read.csv('/Users/tunder/Dropbox/python/genres/results/sanstopicmap.tsv', sep = '\t')
...
p <- ggplot(gdata, aes(x = xcord, y = ycord, color = meandate)) +
  ...
  labs(x = '', y = '') +
  geom_text_repel(aes(xcord, ycord, label = genre), force = 4, box.padding = 0.1, 
...
tiff('/Users/tunder/Dropbox/python/genres/images/sanstopicmap.tiff', height = 8, width = 8, units = 'in', res=400)
plot(p)
dev.off()
plot(p)